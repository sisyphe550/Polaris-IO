...
// GetUserInfo 获取用户信息
// 使用 sqlc 缓存，优先从 Redis 读取
func (l *GetUserInfoLogic) GetUserInfo(in *pb.GetUserInfoReq) (*pb.GetUserInfoResp, error) {
	// 根据用户 ID 查询用户信息（使用缓存）
	user, err := l.svcCtx.UserModel.FindOneWithCache(l.ctx, uint64(in.UserId))
	if err != nil {
		if err == model.ErrNotFound {
			return nil, errors.Wrapf(xerr.NewErrCode(xerr.USER_NOT_EXIST), "userId:%d not found", in.UserId)
		}
		return nil, errors.Wrapf(xerr.NewErrCode(xerr.DB_ERROR), "FindOneWithCache err:%v, userId:%d", err, in.UserId)
	}
